...
void printhistogram(int* arr, float total) {
    for (int i = 0; i < 16; i++)
        printf("%u-%u: %.2f%%\n", i * 16, 16 * i + 15, arr[i] / total * 100);
}
...
{
    int x = strlen(text);
    char * only_number = dec_bin(x);
    char * only_text = string_to_binary(text);

    char * bin_text = malloc((x*8+8+1)*sizeof(char));
    bin_text[0]='\0';
    ...
    //printf("text: %s\n", text);
    //printf("binary string to encode:\n<%s>\n", bin_text);
...
void decode (char * filename)
{
    ...
    char * text = malloc(val+1);
    text[0]='\0';
    int counter = 8;

    for (int i=0; i<val; i++)
    {
        char * bin_ch = malloc(8+1);
        bin_ch[0]='\0';
        for (int j=0; j<8; j++)
        {
            fread(&temp, sizeof temp, 1, fp);
